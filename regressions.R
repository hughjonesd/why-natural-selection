...
run_mediation_analysis_rosla <- function (famhist, score_names, res_all) {
  ...
  # How others did it:
  # Participants born between September 1956 and August 1958. ROSLA= Raising of
  # the school leaving age. Estimated using robust linear regression, with
  # standard errors clustered by year and month of birth. All estimates adjust
  # for the month of birth and sex. The same sample was used for both the
  # conventional linear regression and ROSLA analyses. Inverse probability
  # weights used to correct for under-sampling of participants who left school
  # at age 15 (weight=1.8857).
  ...
  # Sept 56-Aug 58
  rosla_subset <- famhist[famhist$dbym %in% -12:11, ]
  
  estimate_mediated_effect <- function (data) {
    reg_nc_rosla <- lm(n_children ~ rosla + sex + birth_mon, data)
    reg_edu_rosla <- lm(age_fulltime_edu ~ rosla + sex + birth_mon, data)
    stopifnot(coef(reg_edu_rosla)[2] > 0)
    effect_nc_edu <- coef(reg_nc_rosla)[2]/coef(reg_edu_rosla)[2]
    
    # regress edu_years on score_name 
    # no controls, we just want correlation
    # effect_nc_edu * beta_edu_ps is mediated correlation with n_children
    fmls_ea <- lapply(
              glue::glue("age_fulltime_edu ~ {score_names} + sex + birth_mon"),
              as.formula
            )
    fmls_ea <- setNames(fmls_ea, score_names)
    beta_edu_ps <- purrr::map(fmls_ea, lm, data = data) %>%
                     purrr::map_dbl(~ coef(.x)[2])
    effect_mediated_nc_ps <- effect_nc_edu * beta_edu_ps
    
    return(effect_mediated_nc_ps)
  }
  
  effect_mediated_mean <- estimate_mediated_effect(rosla_subset)
  effect_mediated_boot <- replicate(99, {
    boot <- rosla_subset %>% slice_sample(prop = 1, replace = TRUE)
    estimate_mediated_effect(boot)
  })
  effect_mediated_ci <- apply(effect_mediated_boot, 1, quantile, c(.025, .975))
  
  # regress n_children on score_name: this gives raw "total effect"
  fmls_nc <- lapply(glue::glue("n_children ~ {score_names}"), as.formula)
  fmls_nc <- setNames(fmls_nc, score_names)
  mods_nc_ps <- purrr::map(fmls_nc, lm, data = rosla_subset)
  beta_nc_ps <- purrr::map_dbl(mods_nc_ps, ~ coef(.x)[2])
  
  
  res <- data.frame(
                      term = score_names, 
                      beta_nc_ps = beta_nc_ps,
                      effect_mediated_mean = effect_mediated_mean,
                      conf.low  = effect_mediated_ci[1, score_names], 
                      conf.high = effect_mediated_ci[2, score_names]
                    )
