...
generations of family size per respondent. We then relate these to 33 different
polygenic scores for a variety of  characteristics. 

Natural selection is taking place on most polygenic scores, and is concentrated
among population subgroups that are under-represented in the sample, including
less-educated people and poorer households. When we weight the sample to correct
for ascertainment bias, many effect sizes are considerably larger.
...
pattern can be explained by economic theories of fertility. In these, higher
...
they may also differ on other unobserved characteristics.

To examine the effects of sampling, we regress respondents' polygenic
scores on their number of children:
...
mean 0, $\beta$ is the expected polygenic score among children of the sample.

Figures \@ref(fig:plot-educ-level) and \@ref(fig:plot-income) plot effect sizes
for each polygenic score among different subgroups of the sample. Figure
\@ref(fig:plot-educ-level) groups respondents by age of completing full-time
education. Figure \@ref(fig:plot-income) groups respondents by household income.
Effect sizes are larger and more significant for the lowest income category, and
for the lowest education category. Note that the overall effect is not a simple
average of the effect among the different subgroups, because polygenic scores 
may also shift respondents between the subgroups. For example, a high 
PGS for educational attainment may increase the age at which a respondent 
leaves school.
