...
These estimates may suffer from ascertainment bias within the UK Biobank sample.
...
polygenic scores on their number of children:
...
subgroups. For example, a high PGS for educational attainment may increase the
...
When we split respondents up by other demographic variables, we get a more
surprising result: the direction of natural selection is different among
different subgroups. 
...
each tercile of age at first live birth. Several effects are strikingly
...
example, educational attainment is selected against; among those with 3 or fewer
sexual partners, it is selected for.

Figure \@ref(fig:plot-with-partner) splits respondents up by whether they
...
Figure \@ref(fig:plot-age-flb) shows the results of regressions for women only,
controlling for age at first live birth. Effect sizes are greatly reduced.
This could be because the regressions exclude childless women. However, in 
`r opp_signed` out of `r length(score_names)` cases, effects actually change
sign when controls are added. The correlation between effect sizes controlling for age
at first live birth, and raw effect sizes, is `r cor_raw_flb`.
...
res_wt_combined <- bind_rows(
  ...
  `Age/Qual/FLB`  = res_wt_flb_weights,
  .id = "Weights"
)
...
rel_sizes <- res_wt_cb_wide %>% 
                ...
                mutate(
                  ...
                  `Age/Qual/FLB` = `Age/Qual/FLB`/estimate_females
                ) %>% 
...
To compensate for ascertainment bias in the UK Biobank sample, we weight 
...
these weighting schemes. We include unweighted regressions for comparison.
Weighted effect sizes are significantly larger in absolute value. Table
\@ref(tab:tbl-res-wt) gives effect sizes as a proportion of unweighted effect
sizes for the census-based weightings.


Effect sizes are reasonably consistent across the geographical and
age/qualification weights, but they are increased again when we weight by age at
first live birth. This suggests that estimates might be further affected by
weighting on other demographic variables. Given that the Biobank sample
seems to be ascertained in ways that reduce estimates of natural selection, 
we view our estimates as likely to be conservative.
...
rel_sizes %>% 
      ...
      add_footnote(paste0(
        ...
        "are shown.\n",
        "Age/Qual/FLB as a proportion of unweighted regressions ",
        "for females only."),
        font_size    = 6, 
        top_padding  = 0,
        left_padding = 0
...
Why do selection effects have *opposite* signs across certain subgroups -- for
example, across older and younger mothers by age at first live birth, or across those
...
two-parent status, the theory could also explain the differently-signed
effects across people with more and fewer sexual partners.
...
artery disease, but PGS underlying these traits are being selected for.
Potentially, natural selection could increase the health burden on modern
societies, but that depends on effect sizes. Existing work argues that these are
small. However, as we show, estimates can be crucially affected by ascertainment
bias in population samples. In addition, we do not know whether estimated effect
sizes of natural selection will increase or decrease as more accurate polygenic
scores are produced. We are also unsure about whether polygenic scores for new
phenotypes will show a similar pattern of natural selection as existing ones. In
short, it is probably too early to tell whether modern natural selection is
substantively important or not.
...
theory is that traits selected for are linked to impulsivity and low time
discount rates, via the channel of early sexual behaviour. The data here provide
some support for this: scores which might plausibly be linked to
impulsivity,like ADHD and age at smoking initiation, are selected for. However,
the "impulsivity" theory is less good at explaining variation in selection
across the full range of scores, including e.g. waist-hip ratio and BMI.
Impulsivity also does not explain the reversed sign of selection among e.g.
older parents. Overall, then, the economic theory is probably the leading
