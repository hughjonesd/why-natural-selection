...
```md
# TODO
* Improve imputation of parents' YOB.
* Selection bias. 
  - You could weight subjects by education level; sex; birth region? 
...
variance 1. Scores for ADHD, substance use, alcohol, Alzheimer's disease,
autism, bipolarity, BMI, body fat, caffeine, cannabis use, coronary artery
disease, eating disorders, waist-hip ratio, major depressive disorder,
neuroticism, and schizophrenia were reverse coded. High scores are therefore
"positive" in the loose sense that their target phenotypes would be considered
...
* *total siblings*, the number of full siblings of the respondent (including
  himself or herself).
...
`r sum(res_all_wide$consistent)` out of `r n_pgs` polygenic scores, all 4 estimates have the same sign.
...
```{r plot-basic-regs, fig.cap = "Effects of polygenic scores on number of siblings/children. Scores marked * are reverse coded.", fig.height = 7}
...
res_all %>% 
      ...
      facet_wrap(vars(dep.var), ncol = 2, scales = "free") +
      coord_flip() +
...
waist-hip ratio, WC (XXX what is it?) and "Hip combined" since they are 
...
```{r plot-educ-levels, fig.cap = "Effect sizes on number of children by age left full-time education", fig.height = 7}
