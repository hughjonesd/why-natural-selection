...
* can we weight men by age at first child, using GHS? If not, may need
  to plot the age_flb weighted regressions separately, against unweighted
  women-only regressions.
* Can we find representative data for weights by n_partners?
```
...
PGS for educational attainment will increase the age at which a respondent 
leaves school.
...
drake::loadd(res_wt_ghs_weights)
...
first live birth.This suggests that estimates might be increased still more by
weighting on other relevant demographic variables. Overall, these results
suggest that previous work may have underestimated the level of selection
occurring in the population. [TODO: who?]
...
To compensate for this, we reweight our preferred weightings by the inverse of
*number of siblings*. 

TODO: 
- weight regressions using "best effort" weights
- look at change over time to confirm your "no change" hypothesis
- look at siblings, again using "best" weights
