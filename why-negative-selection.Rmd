...
* Another point is that negative selection only among poor will increase
    genetic inequality.
  - one way to look at this: regress PGS on e.g. income or Quin71
  - then weight by n_children and see the change
  - needs to be comparable in some sense though, across generations
  - e.g. quintile of where born, unweighted; vs quintile of where now,
    weighted by children
  - perhaps simpler is just to run regressions with/without weights
    by n_children. This gives you the "effect" of natural selection
...
```{r plot-means-over-time, fig.height = 6, fig.cap = "Mean polygenic scores by birth year in UK Biobank. Points are means for 5-year intervals. Lines are 95% confidence intervals. Green triangles show a significant linear increase over time (p < 0.05/33). Red squares show a significant decrease."}
...
PGS for educational attainment may predict having fewer children among
early school leavers, but it may also increase the age at which a
respondent leaves school.

```{r plot-income-educ-level, fig.cap = "Selection effects by education and income. Each point represents a single bivariate regression of number of children on a PGS", fig.subcap = c("Age left full-time education", "Household income"), fig.ncol = 1}
...
```{r plot-siblings-townsend, fig.cap = "Selection effects in the parents' generation by area Townsend deprivation quintile", fig.align = "center"}
...
[@townsend1987deprivation]. Results (Figure \@ref(fig:plot-siblings-townsend))
show the same pattern as for respondents: effect sizes are larger and more
significant in the most deprived areas.
...
selection has a substantively important effect on populations' mean 
polygenic scores.

Because selection effects are concentrated in poorer populations, they also increase
inequality with respect to polygenic scores. For example, Figure 
\@ref(fig:plot-mean-EA3-by-income) graphs children's mean polygenic scores for 
educational attainment (EA3), in households of different income groups. The blue
bars show the actual means. The grey bars show the hypothetical means if all 
households had equal numbers of children. Natural selection against EA3, which
lowers the mean, is stronger at the bottom of the income distribution, and this
increases the differences between groups.

```{r plot-mean-EA3-by-income, fig.align = "center", fig.cap = "Mean PGS for educational attainment (EA3) of children by household income group. Blue is actual. Grey is hypothetical in the absence of selection effects.", fig.width = 4}
income_EA3 <- famhist %>% 
...
generation (*number of children*), where respondents with no
