...
```{r plot-means-over-time, fig.height = 5, fig.cap = "Mean polygenic scores by birth year in UK Biobank. Points are means for 5-year intervals. Lines are 95% confidence intervals. Green lines show a significant increase over time (p < 0.05). Red lines show a significant decrease."}
...
To test this, we measure the correlation of each PGS with household income, 
as well as with education level, a phenotype which predicts earnings. We
...
fertility, and its correlation with income or education (Figure \@ref(fig:plot-cors-income-educ)). If we control for age at first live birth,
the relationship changes sign and becomes positive.


```{r plot-cors-income-educ, fig.cap="Effect sizes on number of children, with and without controls for age at first live birth, by correlations with income and education"}

drake::loadd(res_all)
drake::loadd(res_age_flb)

cors_income <- cor(famhist[score_names], famhist$income_cat, use = "pairwise")
...
dfr <- data.frame(
        Correlation  = rep(c("Income", "Education"), each = 33, 2),
...
at birth: $\rho$ `r cor_raw_fath`; mother's age at birth: $\rho$ `r
cor_raw_moth`).
