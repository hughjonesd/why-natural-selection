...
n_in_regs <- function (var, dv = "n_children") {
  fh_subset <- famhist
  if (dv == "n_children") fh_subset %<>% filter(age_at_recruitment >= 45)
...
Suppose also that education and innate human capital are complements. They
...
waist-hip ratio and BMI (see also appendix Figure \@ref(fig:plot-together)). 
Externalizing behaviour also does not explain
...
```{r plot-n-partners-controlled, fig.cap = "Selection effects by number of sexual partners (median split at 3), controlling for respondent's age and age squared."}
