...
rel_sizes <- res_wt_cb_wide %>% 
                ...
                mutate(
                  across(Geographical:`Age/Qual/FLB`, ~ .x/None)
                ) %>% 
                select(- consistent, - None, - p.value) %>% 
                mutate(
                  term = pretty_names(term)
                )
...
rel_sizes_summary <- rel_sizes %>% 
                summarize(
                  across(Geographical:`Age/Qual/FLB`, ~ c(mean(.x), median(.x)))
                ) %>% 
                mutate(
                  term = c("Mean", "Median")
                )
...
Figure \@ref(fig:plot-res-wt) shows the results. Effect sizes are significantly
larger in absolute value. Table \@ref(tab:tbl-res-wt) gives effect sizes as a 
proportion of unweighted effect sizes.

```{r tbl-res-wt}
rel_sizes %>% 
      arrange(desc(`Age/Qual/FLB`)) %>% 
      ...
      set_background_color(final(2), everywhere, "grey80") %>% 
      add_footnote(
        "Only consistent and significant (when unweighted) estimates are shown",
        font_size   = 8, 
        top_padding = 2
      )
...
res_wt_combined %>% 
  mutate(
    Weights  = fct_relevel(Weights, "Geographical", "Age/Qualification",
                "Age/Qual/FLB", "None"),
    term     = pretty_names(term),
    term     = fct_reorder(term, estimate, order_abs(4))
  ) %>% 
  ggplot(aes(x = estimate, y = term, shape = Weights, colour = Weights)) +
  geom_point(size = 2) +
  my_vline +
  scale_colour_manual(values = c(
    "None"              = "grey25", 
    "Geographical"      = "darkgreen",
    "Age/Qualification" = "chocolate4",
    "Age/Qual/FLB"      = "midnightblue"
  )) +
  scale_shape_manual(values = c("None" = "square", "Geographical" = "circle",
        "Age/Qualification" = "circle", "Age/Qual/FLB" = "circle"))
