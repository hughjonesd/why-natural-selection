...
author: "David Hugh-Jones \\thanks{Corresponding author. School of Economics, University of East Anglia. Email: D.Hugh-Jones@uea.ac.uk}, Abdel Abdellaoui \\thanks{Amsterdam UMC. Email: a.abdellaoui@amsterdamumc.nl}"
date: "`r format(Sys.Date(), '%d %B %Y')`"
abstract: "Natural selection has been documented in contemporary humans, but  little is known about the mechanisms behind it. We test for natural selection on 33 polygenic scores over two generations, using data from UK Biobank (N = 409,629 British subjects with European ancestry). Consistently over time, polygenic scores which predict lower (higher) earnings and education are selected for (against). Selection effects are concentrated among low-income and less educated people, younger parents, people with more lifetime sexual partners, and people not living with a partner. Effect sizes are substantially larger after correcting for ascertainment bias. The direction of natural selection is reversed among older parents, or after controlling for age at first live birth. These patterns can be explained by economic theories of fertility, in which higher earnings may either increase or decrease fertility via income and substitution effects in the labour market. We find evidence in modern day Great Britain for multiple natural selection pressures that vary between subgroups in the direction and strength of their effects and that are strongly related to the socio-economic system."
...
header-includes:
  ...
  - \titleformat*{\subsection}{\bfseries\sffamily\itshape\Large}
...
As yet we know little about the social mechanisms behind these effects. This study uses
data from UK Biobank [@bycroft2018uk] to learn more. We test for natural selection on 33
...
population, but also emigration, or ascertainment bias within the sample. 
Respondents have higher income and are better educated than the UK population, 
and they may also differ on other unobserved characteristics [@10.1093/aje/kwx246].
Since richer and educated people also live longer, this bias could also
increase with age.
...
(PGS) on their number of children ($y_i$):
...
These results could be driven by age, if older respondents have lower income
and are less educated, and also more subject to selection on polygenic
...
is only significant for the educational attainment PGS of @lee2018gene
("EA3"). One possible reason for these sex differences is
...
oldest terciles is `r cor_age_flb`. As before, recall that polygenic scores may 
have two effects: changing the number of children within each category, and 
changing the number of children by shifting respondents between categories.
...
*controlling* for AFLB (Figure \@ref(fig:plot-age-flb)). In
...
```{r plot-age-flb, fig.cap = "Selection effects controlling for age at first live birth (women only). Effect sizes without controls (for women only) are shown for comparison."}
...
broad-scale genetic variation captured by principal components, such as ancestry.
...
@becker1976child]. According to this theory, increases in a person's wage affect their
...
polygenic score, controlling for earnings in first job and education levels. Effect sizes
...
earnings and education indeed mediate the effect of polygenic scores on fertility.
...
measure of risk attitude (UK Biobank field 2040). While risk attitude is always a
...
and more lifetime sexual partners. Indeed, among older parents, selection effects are
...
Polygenic scores which correlate with high (low) earnings and more (less) education are being
selected against (selected for). In addition, many of the phenotypes under
positive selection are linked to disease risk, or are what many people would see
...
Because selection effects are concentrated in poorer groups, they may also increase
...
scores, could help to pin down directions of causality. Conversely, theory 
and empirical results from economics can shine a light on the mechanisms 
...
Abdel Abdellaoui is supported by the Foundation Volksbond Rotterdam and by ZonMw grant 849200011 from The Netherlands Organisation for Health Research and Development. This study was conducted using UK Biobank resources under application number 40310.
...
regressions interacting polygenic scores with birth year, median split at 1950
...
evidence for change in natural selection within the parents' generation. In the
respondents' generation, effect sizes were significantly larger in absolute size 
among the late-born for four polygenic scores: cognitive ability, EA2, EA3 and
...
Three scores showed significant changes in sign: alcohol use
...
from bivariate regressions for respondents, and Figure
\@ref(fig:plot-age-birth-parents-dv) does the same for their parents. Effect
sizes are reasonably large. They are also very highly correlated across
generations. Effect sizes of polygenic scores on father's age at own birth, and
on own age at first live birth, have a correlation of `r cor_flb_fath`; for
mother's age and own age it is `r cor_flb_moth`.
