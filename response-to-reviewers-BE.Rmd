...
As described above, we have shortened Section 2 to focus on the key results of variation by income, education, number of partners, partner presence and AFLB. These empirics now take up just 39 lines of text, plus Figures 3-5.
...
We experimented a lot with these figures, and they may not be easy to improve. For example, we tried using symbols, but in our judgment the added visual noise makes it harder to see the pattern of the results across rows. Below we show three alternative versions of one of our figures: our original, a version using symbols, and one with subtle tweaks to colors and stroke sizes. We prefer our tweaked version (and have used it in the revision), but we will bow to the reviewers or editor.
