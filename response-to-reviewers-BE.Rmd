...
shapes_ggplot <- function (
  ...
  dfr %>% mutate(
    ...
    scale_color_manual(
                         values = rev(c("lightblue3", "dodgerblue2", "dodgerblue4"))
                       ) +
...
Code to produce this paper is now publicly available at <https://github.com/hughjonesd/why-natural-selection>.
...
We thank the reviewer for giving us this reference. Indeed it is striking. Fisher's theory about the positive correlation he mentions is that it's driven by selection bias: Yale parents who can afford to send 6 children to Yale are "more able" than those who can only send 1, and as a result, among Yale students, children from bigger families are smarter. This is related to the income effect in our theory. We now mention this work in our conclusion.
...
The functional form is a standard one in economics. It has the specific property that relative risk aversion is constant. That is, if you have CRRA utility and £100, and you are indifferent to a 50-50 bet between winning £30 and losing £20, then if you had £1000 you would be indifferent between a 50-50 to win £300 or lose £200.
...
Bootstrapping ratios is not so simple, since if the confidence interval for the denominator contains 0, the ratio confidence set may contain infinity and may not be a single interval, and the standard approach of taking quantiles can be misleading (see Franz 2007). Alternatives like the Fieller method have the same problem. Another issue is that to estimate Bonferroni-corrected significance at p = 0.05/33 we'd need 0.00075- and 0.99925-quantiles of the distribution, which would require many thousands of bootstraps (unfeasibly slow, given our sample size).
...
We now use RLRS as the dependent variable for both respondents and parents. Results are not much changed, except there is now slightly more evidence for larger selection effects over time: 9 polygenic scores show a significant increase in effect size on respondents' RLRS when we median-split them by birth year. In turn, this makes us less sure that we can rule out that the welfare state is a relevant explanation for natural selection, so we have slightly qualified our comments on this topic.
...
We used 45 years old. We have adjusted this to 50 male/45 female as suggested. Results are little changed, except that EA3 is no longer undergoing diversifying selection in the respondents' generation at the p = 0.05/33 significance threshold.
...
We experimented a lot with our basic figures, and they may not be easy to improve. The graphs' key message is "effect sizes in one category are larger than others". To see that the eye needs to be able to quickly differentiate between categories. We have tried several variants using symbols, but in our judgment the added visual noise makes it harder to follow the categories across rows. The figure below shows three alternative versions: our original, a version using symbols, and one with subtle tweaks to colors and stroke sizes, which we hope fix the greenness issue and make p-values clearer. We prefer our tweaked version (and have used it in the revision), but we will bow to the reviewers or editor. For comparison, a version of the whole paper using the symbols plot is available [here](https://raw.githubusercontent.com/hughjonesd/why-natural-selection/alt-plots/why-natural-selection-weighted.pdf).

We've worked on Figures 1 and 2 to improve readability.

```{r different-plots, echo = FALSE, fig.cap = "Alternative plot styles", fig.subcap = c("Original", "Symbols", "Tweaked"), fig.ncol = 1, fig.height=2.5, fig.align="center"}
