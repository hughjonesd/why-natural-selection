...
-   Code to reproduce this paper is now publicly available at <https://github.com/hughjonesd/why-natural-selection>.
...
bound as income approaches zero, $\lim_{x\rightarrow0}u'(x)=\infty$ )". It can be derived in this case by differentiating $u$ to give $u'(y) = y^{-\sigma}$ which for $\sigma > 0$ approaches infinity as $y \rightarrow 0$. An intuitive interpretation is that if you have extremely low income, your need for income overrides other considerations.
...
We agree with this. Combining all the variables is hard: age at first live birth is only available for females, and some of the data comes from different sources. Our ideal option is to use the weights that are being developed by Sjoerd van Alten and coauthors. Their working paper is specifically dedicated to weighting UK Biobank, so it seems good to use the "industry standard" solution rather than rolling our own. We now use these weights throughout. XXX
...
We experimented a lot with our basic figures. The graphs' key message is "effect sizes in one category are larger than others". To see that the eye needs to be able to quickly differentiate between categories. We have tried several variants using symbols, but in our judgment the added visual noise makes it harder to follow the categories across rows. The figure below shows three alternative versions: our original, a version using symbols, and one with subtle tweaks to colors and stroke sizes, which we hope fix the greenness issue and make p-values clearer. We prefer our tweaked version (and have used it in the revision), but we will bow to the reviewers or editor. For comparison, a version of the paper using the symbols plot is available [here](https://raw.githubusercontent.com/hughjonesd/why-natural-selection/alt-plots/why-natural-selection-weighted.pdf).
