...
among full siblings, controlling for a full set of sibling group dummies
(Appendix Figure \@ref(fig:plot-res-fe-fertility)). On the one hand, all 
within-sibling effects are insignificant after Bonferroni correction. On the
other hand, effect sizes are positively correlated with effect sizes from the
pooled model, and about 1/3 smaller (regressing within-sibling on pooled effect
sizes, b = `r coef_pool_w`, se = `r se_pool_w` ). We view this as providing weak
evidence that polygenic scores are causally linked to fertility, with at least
some role for correlated environments. Also, if we add a control for education
to the within-sibling regressions, effect sizes barely change. This suggests
that our measure of education only mediates differences in fertility between
sibling groups; among siblings, other causal channels are more important.
...
10^{-16}$ in `r n_risk_sig`/33 regressions). Given that, and the statistical power
