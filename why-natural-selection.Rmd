...
standard_ggplot <- function (
  ...
  dfr %>% mutate(
    ...
    my_labs() +
    my_theme(legend.justification = "top") +
    confint_scale +
...
my_fill_scale <- function (n, aesthetics = "fill", direction = 1, ...) {
  ...
  scale <- if (n <= 2) {
    ...
    fill_colors <- c(fill_colors[1], "skyblue", fill_colors[2])
...
res_mediation %>% 
      ...
      ggplot(aes(y = term)) + 
        ...
        scale_fill_manual(values = c("FALSE" = "grey65", "TRUE" = "blue3")) +
        my_vline +
        scale_x_continuous(labels = scales::percent, n.breaks = 7) +
        coord_cartesian(xlim = c(-0.5, 1)) +
        labs(x = "Proportion mediated", y = "") +
        scale_linetype_manual(name = "", values = c("Bootstrap 95% c.i. uncorrected" = 1))
