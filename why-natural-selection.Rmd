...
```{=tex}
\begin{equation}
RLRS_i = \alpha + \beta\mathrm{PGS}_i + \varepsilon_i
\label{eq:regression}
\end{equation}
```
...
[^normalization]: XXX The selection effect $\beta$ equals $Cov(N, PGS)/Var(PGS)$
...
in the next generation [@robertson1966mathematical].
...
We also check for stabilizing and disruptive selection by estimating
\@ref(eq:regression) with a quadratic term.[^balance-diverse-def] In particular,
we find disruptive selection for educational attainment polygenic scores: at
higher values of these scores, the negative effect on fertility is smaller
(Appendix Figure \@ref(fig:plot-purifying)).

[^balance-diverse-def]: Stabilizing selection selects for intermediate values, 
reducing the variance of the trait under selection. Disruptive selection
...
we find little evidence of change in effect sizes among either the parents' or
the respondents' generation. We also check whether selection effects vary by
...
\@ref(tab:tbl-res-mediation)). For `r n_pos_sig` scores, the
indirect effect of the score on fertility *via* educational attainment is
significantly different from zero ($p < 0.05/33$) and takes the same sign as the
overall effect. Among these scores, the median proportion of the total effect
explained by the indirect effect is `r percent(median_prop)`. Figure
\@ref(fig:plot-mediation) shows estimated proportions explained along
with bootstrap 95% confidence intervals (uncorrected; 100 bootstraps).
The educational attainment variable is a relatively crude measure of human 
capital: more accurate measures would likely explain more of the total effect.
...
The EA2 score for educational attainment shows significant disruptive
...
Scores for EA2 and EA3 show significant disruptive selection ($p <
0.05/33$, positive coefficient on quadratic). Other scores including 
hip circumference were not significant.
...
To learn more about this, we compare effect sizes excluding and including childless
...
summarize the results. We report the number of scores showing significant
...
summary_period %>% 
      ...
      count() %>% 
      period_table("Change in selection effects between parents of early and late born respondents (regressions on parents' RLRS). Weights: Age/Qualification times 1/number of siblings.")
...
summary_period %>% 
      ...
      count() %>% 
      period_table("Change in selection effects between early and late born respondents (regressions on RLRS). Weights: Age/Qualification.")
```
