---
title: "Human capital drives natural selection in contemporary humans"
...
* Formalize theory - done.
  - Comment on results where effects are actually reversed. Could this
  happen if \sigma were higher in period 2? Or if it increased with income?
  Could maybe do a computational example.
  - One possibility is CARA utility. Solving it computationally
* Clarify causality?
...
[@beauchamp2016genetic].[^normalization] 
...
naïve estimates of natural selection are at risk of
...
further affected by weighting on other demographic variables.

We also check for purifying and diversifying selection by 
estimating \@ref(eq:regression) with a quadratic term. See Appendix Figure \@ref(fig:plot-purifying). In particular, we find diversifying selection for
educational attainment polygenic scores: at higher values of these scores, 
the effect on fertility is closer to zero.
...
To understand the reasons for this ascertainment bias, and to learn about the
mechanisms underlying natural selection, we split the sample, starting with
...
We emphasize two points. First, this exercise does
*not* apportion the total correlation with fertility between subgroups. 
That is because the polygenic score may also predict the probability of being in 
...
a person's chance of attending university. Second, we do *not* claim that our
observed correlations reflect causal effects of polygenic scores on fertility.
Indeed, polygenic scores correlate with many sources of environmental variation 
that could affect fertility. However, natural selection is a correlational
phenomenon, not a causal one. That is, if a polygenic score correlates with 
low fertility, it is being selected against, whatever the underlying causal
relationship. Differences in equation \@ref(eq:regression) across subgroups can 
be informative about how natural selection works, because they constrain the set 
of possible explanations to those that generate these differences.
...
income category, and for the lowest education category. 
...
scores. However, when we rerun the regressions, interacting the polygenic
...
cause opposing effects on fertility. The income effect will lead to natural
selection in favour of earnings-increasing variants. The substitution effect
will do the reverse. 

To demonstrate this, we write down a simple model of 
fertility choices. $h$ is an individual's level of human capital, which 
(for now) we identify with his or her wage $W$. Raising a child takes time $b$. People maximize utility
from the number of children $N$, and income $Y\equiv(1-bN)W$. An
...
Here $a$ captures the strength of preference for children. $u(\cdot)$
is concave and increasing. We treat $N$ as continuous, in line with
...
$\frac{dU}{dN}=-bWu'(Y)+a$. The effect of an increase in wages (or human capital) 
on this marginal benefit is

\[
\frac{d^{2}U}{dNdh}=\underbrace{-bu'(Y)}_{\textrm{Substitution effect}}\underbrace{-bYu''(Y)}_{\textrm{Income effect}}.
\]

The *substitution effect* is negative and reflects that when
wages increase, time devoted to childcare costs more in foregone income.
The positive *income effect* reflects that when wage income is
higher, the marginal loss of wages from children is less painful.
Letting $u(\cdot)$ take the constant relative risk aversion (CRRA)
form $u(y)=\frac{y^{1-\sigma}-1}{1-\sigma}$, where $\sigma>0$ captures
risk aversion in income, we solve for optimal fertility $N^{*}$,
and examine the *fertility-human capital relationship*, $\frac{dN^{*}}{dh}$.
For moderate levels of risk aversion ($0.5<\sigma<1$):

\begin{enumerate}
\item The fertility-human capital relationship is negative: $\frac{dN^{*}}{dh}<0$.
\item The relationship is weaker (closer to zero) at higher wages and/or
levels of human capital.
\item The relationship is more negative when the time burden of children,
$b$, is larger.
\end{enumerate}
...
Instead of identifying human capital $h$ with wages, we now allow
individuals to choose a level of education $s\in[0,1]$, which has
a time cost in period 1. Education increases period 2 wages, and is
complementary to human capital $h>0$. For period 2 wages we assume
the simple functional form $w(s,h)=sh$. We normalize period 1 wages
to 1. Maintaining the assumption of CRRA utility, we examine total
fertility $N^{*}\equiv N_{1}^{*}+N_{2}^{*}$. For $0.5<\sigma<1$,
facts 1-3 continue to hold (fact 2 in a neighborhood of $\sigma=1$).
In addition, for $\sigma$ in a neighborhood of 1:

\begin{enumerate}
\item [4.]\setcounter{enumi}{4}The fertility-human capital relationship
is weaker at higher levels of education $s$.
\item The relationship is weaker among those who start fertility in period
2 ($N_{1}^{*}=0$) than among those who start fertility in period
1 ($N_{1}^{*}>0$).
\end{enumerate}

Facts 1-5 match our empirical results. In this model, polygenic scores which
correlate with human capital would have a negative correlation with fertility at
low income and education levels, and a weaker or zero correlation at higher
levels (Figure \@ref(fig:plot-income-educ-level)). For scores which are highly
correlated with human capital, the correlation would also be weaker at higher
levels of the score itself, giving rise to diversifying selection (Appendix
Figure \@ref(fig:plot-purifying)). The correlation would be more negative when
the time burden of children $b$ is high, e.g. for single parents (Figure
\@ref(fig:plot-n-partners)). Lastly, the correlation would be weaker among those
starting fertility later (Figure \@ref(fig:plot-age-flb)).

Our results are actually stronger than this, in that correlations with
fertility are reversed at higher AFLB. This could be accommodated in the model 
if children have a money cost as well as a time cost, since money costs are less
burdensome at higher wages, i.e. in period 2 for those with high human capital. Appendix Figure \@ref(fig:N-plot-with-m) shows an example.
...
The above does not provide a complete theory of fertility. Rather, it shows that
a relatively simple economic model can explain many of our results. Other
empirical work in economics also supports our mechanism.
...
mothers [@baudin2015fertility], in line with our fact 3 and as predicted by
@becker1981treatise.
...
have children (parameter $a$ in the model; @jones2008fertility). However, this
theory would not explain why our results are weaker at higher incomes and
education levels. Indeed, in the one-period model, while $\frac{dN^{*}}{da}>0$,
this relationship actually becomes stronger at higher wages.

Another alternative is that traits under selection are linked to externalizing
behaviour and risk-seeking. This might be captured by our risk-aversion 
parameter $\sigma$, which affects fertility by changing the marginal utility of income, but a more direct channel is risky sexual behaviour
...
scores which are significant at $p \le 0.05/33$ in uncontrolled regressions
remain so when controlling for risk attitude. This non-result could simply
reflect the imprecision of the risk aversion measure, which is a single yes/no
question. However, the risk aversion measure *does* predict the overall number
of children, highly significantly ($p \le 2 \times 10^{-16}$, `r n_risk_sig`/33
regressions). Given that, and the statistical power we get from our large N, we 
believe that the non-result is real: while risk aversion does predict fertility 
in the sample, it is not an important channel for natural selection.
...
phenotypes under positive selection are linked to disease risk. Most people
...
\@ref(tab:tbl-inequality-effects)). These are substantial effects, and since
...
```{r plot-res-earn-educ, fig.cap = "Selection effects controlling for earnings in first job (estimated by mean earnings from ASHE 2007 for the SOC2000 job code) and education (left education before 16, 16-18, or after 18). Raw effects are shown for comparison."}
