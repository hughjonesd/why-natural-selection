...
cmp %>%
      mutate(
        term = fct_reorder(term, estimate.x, order_abs(1)),
        term = pretty_names(term)
      ) %>%
      ggplot(aes(y = term)) +
        geom_pointrange(aes(x = estimate.x, xmin = conf.low.x,
                            xmax = conf.high.x), color = "red", alpha = .8) +
        geom_pointrange(aes(x = estimate.y, xmin = conf.low.y,
                            xmax = conf.high.y), color = "blue", alpha = .8)
...
Unfortunately, however, absolute effect sizes are smaller when the
...
people have such a strong effect on estimates makes it hard to compare absolute
...
As an alternative approach, , we run
