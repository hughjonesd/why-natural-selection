...
first live birth. Figure \@ref(fig:plot-res-wt) plots selection effects among the
...
and in a neighbourhood of $\sigma = 1$, Table \@ref(tab:theory) shows five theoretical 
...
2.  The fertility-human capital                       Figure \@ref(fig:plot-income-educ-level)a. 
    ...
    at higher wages and/or levels of                  polygenic scores for EA (Appendix Figure
    human capital.                                    \@ref(fig:plot-purifying)).
...
generally reduced (Appendix Figure \@ref(fig:plot-res-earn-educ)), with
large reductions for polygenic scores for educational attainment. This 
suggests that earnings and education indeed mediate the effect of 
polygenic scores on fertility.
...
cor_pool_within <- cor.test(res_all$estimate.x, res_all$estimate.y)
```

One interpretation of these results is that natural selection on polygenic
scores is driven by their correlation with human capital, which affects
fertility as our model describes. This does not necessarily imply that polygenic
scores are causal of either human capital or fertility. Instead they may simply
correlate with different family environments, for example via "genetic nurture"
effects [@kong2018nature] or population stratification. To test whether
polygenic scores cause fertility, we estimate equation \@ref(eq:regression)
among full siblings, controlling for sibling group fixed effects (Appendix
Figure \@ref(fig:plot-res-fe-fertility)). Results are never significant after
Bonferroni correction. The correlation between pooled and within-sibling effect
sizes is `r cor_pool_within$estimate` (p = `r cor_pool_within$p.value`), and within-sibling effects are about 1/3 smaller. Thus, there is only weak evidence for causal
links from polygenic scores to fertility, with at least some of the effect
coming from gene-environment correlation. If we re-estimate the within-sibling
regressions adding a control for education, effect sizes barely change. This
suggests that our measures of education only mediate between-sibling-group
differences in fertility; among siblings, other causal channels are more
important.
...
* Results:
  - in bivariate within-sib regs, extraversion, EA2/3, ADHD and some others are
    weakly significant (only at .05!)
  - adding education as a control barely changes effect sizes
  - adding income as a control increases effect sizes (of EA at least)
  - by contrast, in pooled regs, adding education shrinks effect sizes, and
    adding income does nothing.
  - relevant: effect of EA2 on education is halved within-sib, and effect of
    education on fertility is halved within-sib
  - whereas, effect of EA2 on income is halved within-sib, but effect of income
    on fertility is multiplied by 10!! within-sib. Much of this is because
    of year of birth confound i.e. declining fertility and also retired people
    have smaller income
  - within-sib and pooled coefs for PGS are highly correlated; within coefs
    are about 67% of the size.
    
* Interpretation:
  - some families are better educated and have fewer children
  - they also have selected-against PG scores like what we see
  - scores aren't necessarily causal for children, they may be
    associated
  - the between-family effect goes through education, but not the 
    within-family effect (cf. the paper on causality)
...
@Rimfeld_2018], then these increases are substantial effects. Since many
...
```{r plot-res-fe-fertility, fig.cap = "Selection effects controlling for sibling-group fixed effects. \"Controlled\" regressions add a control for education (left education before 16, 16-18, or after 18)."}
