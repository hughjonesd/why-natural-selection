...
my_fill_scale <- function (n, aesthetics = "fill", ...) {
  ...
  if (n <= 2) {
    ...
    shades::lightness(scale_color_brewer(aesthetics = aesthetics, ...), 
                        shades::scalefac(0.85))
  }
}
...
```{r plot-means-over-time, fig.height = 6, fig.cap = "Mean polygenic scores (PGS) by birth year in UK Biobank. Circles are means for 5-year intervals. Lines are 95% confidence intervals. Solid circles show a significant linear increase (green) or decrease (red) over time (p < 0.05/33)."}
...
pgs_over_time %>% 
      ...
      geom_linerange(aes(ymin = score_lo, ymax = score_hi), color = "grey60", 
                       size = 3) +
      geom_point(aes(y = score), size = 2) +
      facet_wrap(~score_name, ncol = 6) +
      my_hline +
      scale_color_manual(values = c("-" = "darkred", "+" = "darkgreen", "o" = "black")) +
      scale_shape_manual(values = c("-" = "circle", "+" = "circle", "o" = "circle open")) +
      theme(
...
lifetime reproductive success (RLRS) on their polygenic scores (PGS):
...
Figure \@ref(fig:plot-res-wt) plots selection effects in the entire
sample.[^balance-diverse-def] To correct for ascertainment bias, we weight
participants using the model of @van2021effects. Weighting makes a
large difference: effect sizes go up by a mean of 
`r percent(mean_rel$Main - 1)`.[^weighting] `n_sig` out of 33 selection effects
are significant at $p$ < 0.05/33.
...
res_wt_combined %>% 
      ...
      scale_fill_manual(values = c(
        "Unweighted"        = "grey15", 
        "Weighted"          = "darkseagreen4"
      )) +
      scale_shape_manual(values = c(
        "Unweighted" = "square filled", 
        "Weighted"   = "circle filled"
      )) + 
...
subgroups in the sample. Splitting the sample by demographic and social
variables, we re-estimate \@ref(eq:regression) within each subgroup. We use
socio-economic variables including income and education, and family structure
variables including age at first live birth, presence of a partner, and lifetime
...
income category, and for the lowest education category. These results are robust
to controlling for respondents' age (Appendix section \@ref(sec:age-control)).
...
Now, selection effects are larger and more significant among
...
We emphasize that our categories are not exogenous to polygenic scores. For
example -- both in the data (Appendix Figure \@ref(fig:plot-age-flb-dv)) and in
our theoretical model -- age at first live birth is a choice variable, which is
endogenous to a person's human capital and their related polygenic scores.
Nevertheless, differences in selection effects across subgroups constrain the
set of possible theories; a good theory of contemporary natural selection needs
to explain how these results come about. As we show below, a model based on the
economic theory of fertility can do just that.
...
grandchildren. We don't find any: in fact, the correlation between respondents' 
...
choose a level of education $s \in [0,1]$, which has a time cost in period 1.
...
$\frac{dN^{*}}{dh}$. For $\sigma < 1$ in a neighbourhood of $\sigma = 1$, Table
...
is balanced by the income effect as income increases.
...
@becker1981treatise. A related literature shows negative correlations
...
younger age at smoking initiation, are selected for. This theory is less good,
though, at explaining variation in selection across the full range of scores,
including physical measures such as waist-hip ratio and BMI. We test the theory
...
But this measure *does* predict the overall number of children, highly
...
or the resulting changes in allele frequencies. Lastly, although effects on
our measured polygenic scores are small, even after weighting, disadvantages which
are individually small may cumulate and/or interact to create larger effects. 
Overall, it is probably too early to tell whether modern natural selection has a 
substantively important effect on population means. 
...
important for understanding social structure and mobility 
...
selection may aid in mapping the genetic architecture of current health
disparities.
...
```{r plot-age-birth-parents-dv, fig.cap = "Effects of polygenic scores on parents' age at respondent's birth, eldest siblings. Weights multiplied by 1/number of siblings."}
...
footnote <- sprintf("* p < 0.05/33. Analysis run on %s PGS which correlated 
                    significantly with fertility.", nrow(res_mediation))
...
effect sizes from the pooled model, and about 50% smaller (regressing
within-sibling on pooled effect sizes, $b$ = 
`r format(coef_pool_w, digits = 3)`). This attenuation is in line with the
~50% decrease in heritability in
within-sibling GWASs on age at first birth and educational attainment
...
```{r plot-regs-controlled-pcs, fig.cap = "Selection effects using unresidualized polygenic scores. Parental generation weights multiplied by 1/number of siblings.", fig.height = 6}
...
res_all %>% 
      mutate(
        ...
        dep.var  = fct_recode(dep.var, 
                                `Respondents' RLRS` = "RLRS",
                                `Parents' RLRS` = "RLRS_parents" 
                              ),
      ...
      ggplot(aes(y = term, x = estimate, fill = reg.type, 
        colour = p < 0.05/{{n_regs}})) +
        my_vline + 
        facet_grid(rows = vars(dep.var), scales = "free") +
