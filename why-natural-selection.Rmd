...
```{r plot-means-over-time, fig.height = 7, fig.cap = "Mean polygenic scores (PGS) by birth year in UK Biobank. Circles are means for 5-year intervals. Lines are 95% confidence intervals. Triangles show a significant linear increase or decrease over time (p < 0.05/33)."}
...
res_wt_combined %>% 
      ...
      standard_ggplot(fill_col = Weights, n_regs = n_regs)
...
pct_diff_edu <- mean_pct_diff(res_edu_wide)
pct_diff_income <- mean_pct_diff(res_income_wide)
pct_diff_n_partners <- mean_pct_diff(res_partners_wide)
pct_diff_with_partner <- mean_pct_diff(res_with_partner_wide)
```
...
education category, and for the lowest income category. The mean percentage
...
significant among those with more than 3 lifetime partners, with a mean percentage
...
interview (Figure \@ref(fig:plot-n-partners)b).  Effects are larger among those not living with a spouse or partner. The mean percentage difference is 
...
that depends on effect sizes. Our results show that naïve estimates can be
...
zero. Researchers should be aware of the risks of ascertainment when studying 
