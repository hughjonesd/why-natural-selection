...
and in a neighbourhood of $\sigma = 1$:


1. The fertility-human capital relationship is negative: $\frac{dN^{*}}{dh} < 0$.
2. The relationship is weaker (closer to zero) at higher wages and/or
  levels of human capital.
3. The relationship is more negative when the time burden of children,
  $b$, is larger.
4. The relationship is weaker at higher levels of education $s$.
5. The relationship is weaker among those who start fertility in period
  2 ($N_{1}^{*} = 0$) than among those who start fertility in period
  1 ($N_{1}^{*} > 0$).


Facts 1-5 parallel our empirical results.[^one-period-results] In this model,
polygenic scores that correlate with human capital would have a negative
correlation with fertility at low income and education levels, and a weaker or
zero correlation at higher levels (Figures \@ref(fig:plot-income-educ-level)a
and \@ref(fig:plot-income-educ-level)b). Among scores which are highly
correlated with human capital, the fertility correlation might also be weaker at
higher levels of the score itself, giving rise to diversifying selection, as we
observe for scores for educational attainment (Appendix Figure
\@ref(fig:plot-purifying)). The correlation would be more negative when the time
burden of children $b$ is high, e.g. for single parents (Figure
\@ref(fig:plot-n-partners)). Lastly, the correlation would be weaker among those
starting fertility later (Figure \@ref(fig:plot-age-flb)).[^money-costs]

[^one-period-results]: Facts 1-3 also hold in the one-period model.

[^money-costs]: Our results are actually stronger than this, in that
...
genetic variants will be selected for (or against) in proportion to their correlation with human capital. Figure \@ref(fig:plot-cors-earnings-educ) plots selection effects
...
10^{-16}$, `r n_risk_sig`/33 regressions). Given that, and the statistical power
