...
confint_geom_segment <- function () {
  geom_segment(aes(x = conf.low, xend = conf.high, linetype = "95% c.i. uncorrected"), 
                    color = "grey55", alpha = 0.25, size = 1.1, 
                    lineend = "round")
}
...
my_fill_scale <- function (n, aesthetics = "fill", ...) {
  ...
  scale <- if (n <= 2) {
    ...
    scale <- scale_color_brewer(aesthetics = aesthetics, ...)
    shades::lightness(scale, shades::scalefac(0.75))
  }
...
res_fe_fertility %>% 
  ...
  standard_ggplot(fill_col = Controls, n_regs = n_regs,
                  fill_direction = -1) +
...
res_all %>% 
      filter(dep.var == "RLRS") %>% 
      standard_ggplot(fill_col = PGS, n_regs = n_regs)
