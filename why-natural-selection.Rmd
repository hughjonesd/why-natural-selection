...
shows that a simple economic model can explain many of our results.
...
Second we test whether educational attainment mediates the correlation between
polygenic scores and fertility. 
...
We consider three alternative theories that might explain our results. The first
is that welfare benefits which incentivize child-bearing might be taken up
...
imprecision of the risk aversion measure, which is a single yes/no question.
However, the risk aversion measure *does* predict the overall number of
...
size, we believe that the non-result is real: while risk aversion does predict
...
`r r2_income_true_psea` among respondents. Counterfactually holding environmental
conditions constant, natural selection would raise this proportion to 
`r r2_income_true_psea_wt` among respondents' children. Similarly, since many
polygenic scores are predictive of disease risk, they could potentially increase
health inequalities. In general, the evolutionary history of anatomically modern
humans is related to disease risk [@benton2021influence]; understanding the role
of contemporary natural selection may aid in mapping the genetic architecture of
...
issue is that we are in effect estimating parents' polygenic scores by the
scores of their children. This introduces some noise into our independent
...
n_positive <- sum(res_mediation$prop > 0)
n_pos_sig <- sum(res_mediation$sig & res_mediation$prop > 0)
median_prop <- median(res_mediation$prop)
...
Table \@ref(tab:tbl-res-mediation) shows results. `r n_pos_sig` scores are 
significantly greater than 0 at $p = 0.05/33$. 
...
had the same number of children (equivalently, the correlation in the
respondents' own generation).
...
remain the same, so long as $\lambda$ dues not differ between the two sets of
weighted regressions.
...
age and qualification) and `r res$r2_income_ea3_fe_wt` (multiplying weights by number of children). Thus, this procedure estimates the proportional change in $R^2$ 
as `r prop_wt_fe`.
