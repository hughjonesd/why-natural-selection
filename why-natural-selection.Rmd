...
welfare benefits which incentivize child-bearing might be taken up more
among low-income individuals. However, our effect sizes appear unchanged
over a large span of twentieth-century history (tables \@ref(tab:tbl-siblings-by-period) and 
\@ref(tab:tbl-children-by-period)), during which
government spending on child-related benefits varied considerably
...
polygenic scores correlate with the motivation to have children, i.e. parameter $a$
in the model [cf. @jones2008fertility]. This theory would not explain why our
results are weaker at higher incomes and education levels.[^note] A third
alternative is that traits under selection are linked to externalizing behaviour
and risk-seeking. This might be partially captured by our risk-aversion
parameter $\sigma$, which affects fertility by changing the marginal utility of
income, but a more direct channel is risky sexual behaviour
...
However, this measure *does* predict the overall number of
children, highly significantly ($p < 2 \times 10^{-16}$ in `r n_risk_sig` out of
33 regressions). Given that, and the statistical power we get from our sample
size, we believe that the non-result is real: while risk attitude does predict
fertility in the sample, it is not an important channel for natural selection.
...
respondents' generation, effect sizes are larger for younger parents. This
holds for both sexes.

```{r plot-age-birth-parents-cross, fig.cap = "Selection effects (parents' generation) by age at first live birth terciles.", fig.subcap = c("Mothers", "Fathers"), fig.ncol = 1}
...
```{r plot-pc-regs, fig.cap = "Selection effects of 100 principal components of genetic data. Each dot represents one bivariate regression. Absolute effect sizes are plotted. Points are jittered on the Y axis. Top principal components are labelled."}
