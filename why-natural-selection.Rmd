...
As yet we know little about the social mechanisms behind these effects. A
potential explanation for them comes from the economic theory of fertility first
presented by [@becker1960economic]. In this theory, higher potential earnings
...
We then show that a parsimonious model of human capital, education and fertility
...
correlation with education and earnings. Furthermore, controlling for education
and earnings reduces the correlation with fertility, suggesting that human
capital mediates natural selection. Thus, contemporary natural selection on
polygenic scores can be explained by their correlation with earnings-increasing
human capital.

Lastly, we discuss the effects of natural selection. While measured effects on
mean genetic scores are small, natural selection may substantially increase the correlation
between genetic scores and income, i.e. the "genetic lottery" [@harden2021genetic].
...
polygenic scores. Nevertheless, differences in equation
\@ref(eq:regression) across subgroups constrain the set of possible explanations
for natural selection. As we show below, the economic theory predicts 
these differences, while alternative explanations do not.
...
We also examine selection effects among respondents' parents, using
information on respondents' number of siblings. Effect sizes of polygenic
scores are highly correlated across the two generations. Median-splitting
respondents by year of birth, we find little evidence of change in effect sizes
among either the parents' or the respondents' generation. We check whether 
selection effects differ by socio-economic status in the
...
[@townsend1987deprivation]. Results show the same pattern as for respondents:
effect sizes are larger and more often significant in the most deprived areas
(Appendix Figure \@ref(fig:plot-siblings-townsend)). Lastly, the siblings data 
lets us check for a "quantity-quality tradeoff" between
number of children (of respondents' parents) and number of grandchildren. We
...
CRRA utility. Our empirical results are actually stronger than prediction 5, in
that correlations with fertility are *reversed* at higher AFLB. This prediction
can be accommodated in the model if children have a money cost as well as a time
cost. Appendix Figure \@ref(fig:N-plot-with-m) shows an example.
...
as providing tentative evidence that polygenic scores cause fertility, with effects
...
educational attainment is a concern. For example, educational attainment might
correlate with unobserved characteristics that affect one's fertility, e.g. via
success in marriage markets. To partially address this, we include
controls for age, sex, fluid IQ and height in $X$.
